...
Page({
  ...
  onLoad(options) {
    const app = getApp();
    // 兼容getApp()为空的情况，避免报错
    if (app && app.getBaseUrl) {
      console.log('【租房详情页】全局基础地址：', app.getBaseUrl());
    }
    console.log('【租房详情页】传入参数：', options); 
    
    // 🔥 紧急修复：恢复调试模式，确保页面能打开
    let { id } = options;
    if (!id) {
      console.warn('房源ID不存在，使用测试ID=1（仅调试用）');
      id = 1; // 强制用测试ID，不返回列表页
      wx.showToast({ title: '使用测试房源数据', icon: 'none' });
      // 暂时注释所有返回逻辑，先让页面能显示
      // setTimeout(() => wx.navigateBack(), 1500);
      // return;
    }
    this.getRentalDetail(id);
    // 🔥 注释掉收藏状态检查，消除404报错（这里之前多了闭合括号，已删除）
